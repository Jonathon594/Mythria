...
public class WorldGenListener {
    ...
    public static void onBiomeLoadingNormal(BiomeLoadingEvent event) {
        List<Supplier<ConfiguredFeature<?, ?>>> underground_ores = event.getGeneration().getFeatures(GenerationStage.Decoration.UNDERGROUND_ORES);
        underground_ores.clear(); //Remove unwanted ore features
        ...
        List<Supplier<ConfiguredFeature<?, ?>>> localModifications = event.getGeneration().getFeatures(GenerationStage.Decoration.TOP_LAYER_MODIFICATION);
        localModifications.add(() -> new ConfiguredFeature<>(MythriaFeatures.GROUND_ROCK_GEN,
                ...
                new FeatureSpreadConfig(FeatureSpread.func_242253_a(4, 8)))));
        ...
        localModifications.add(() -> new ConfiguredFeature<>(MythriaFeatures.GROUND_FLINT_GEN,
                new GroundCoverConfig(0.2f, MythriaBlocks.GROUND_FLINT.getDefaultState()))
                .withPlacement(new ConfiguredPlacement<>(MythriaPlacements.GROUND_COVER,
                        new FeatureSpreadConfig(FeatureSpread.func_242253_a(4, 8)))));
        ...
        if(name.equals(Biomes.CRIMSON_FOREST.getLocation()) || name.equals(Biomes.WARPED_FOREST.getLocation())) {
            event.getSpawns().getSpawner(EntityClassification.CREATURE).add(
                    new MobSpawnInfo.Spawners(MythriaEntities.NETHER_CHICKEN.get(), 1, 2, 6));
        }
    }
}
