...
public class CampfireBlock extends MythriaBlockContainer implements IWaterLoggable {
    ...
    public static final IntegerProperty FULL = IntegerProperty.create("full", 0, 3);
            ...
            if (state.get(LIT)) {
                ...
                if (optional.isPresent()) {
                    ...
                    if (profile.hasFlag(AttributeFlag.BASIC_COOKING)) {
                        if (!isRemote && campfiretileentity.addItem(player.abilities.isCreativeMode ? itemstack.copy() : itemstack, optional.get().getCookTime())) {
    ...
    public boolean receiveFluid(IWorld worldIn, BlockPos pos, BlockState state, FluidState fluidStateIn) {
        if (!state.get(BlockStateProperties.WATERLOGGED) && fluidStateIn.getFluid() == Fluids.WATER) {
            ...
            if (flag) {
                if (worldIn.isRemote()) {
                    for (int i = 0; i < 20; ++i) {
                        spawnSmokeParticles((World) worldIn, pos, state.get(SIGNAL_FIRE), true);
                    }
                } else {
    ...
    protected void fillStateContainer(StateContainer.Builder<Block, BlockState> builder) {
        builder.add(FULL, LIT, SIGNAL_FIRE, WATERLOGGED, FACING, CHARCOAL);
    }
